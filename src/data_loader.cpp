...
Rcpp::List matchscreen(std::string screenname, std::string stringname1,
  ...
  cout << "size of SA11_:" << SA11_.size() << endl;
  cout << "size of SA11_r:" << SA11_r.size() << endl;
  ...
  //------------------------------------------------------------------------------------//
  fvec bh1 = bhat1.elem(idx);
  fvec s12_ = se12.elem(idx);
  
  // fvec sample_size_ = samsize.elem(idx);
  
  
  // cout<<"se12:"<< se12.t()<<endl;
  // cout<<"s12:"<< se12.elem(idx).t()<<endl;
  ...
  //------------------------------------------------------------------------------------//
  fvec bh2 = bhat2.elem(idx);
  fvec s22_ = se22.elem(idx);
  ...
  //
  List output = List::create(Rcpp::Named("rsname0") = rsname0,
                             Rcpp::Named("bh1") = bh1,
                             Rcpp::Named("bh2") = bh2,
                             Rcpp::Named("s12") = s12_,
                             Rcpp::Named("s22") = s22_,
...
Rcpp::List matchsnp(std::string stringname1, std::string stringname2, std::string stringname3){
  ...
  cout << "Number of SNPs (summarystat 1):" << Ns1 << endl;
  ...
  cout << endl;
  printf("Start loading summary stat 1: \n");
  t1 = clock();
  Read_summarystat(stringname1, SA11, SA12, rsname1, betah1, s12, PV1, chr1, bp1, Ns1);
  cout << "Finish loading summary stat 1 in " << (clock() - t1)*1.0 / CLOCKS_PER_SEC << " sec." << endl;
  //----------------------------------------------------------------------------------------------//
  // summary stat SNP info for trait 2, betah, s2, sample_size
  int Ns2 = getLineNum(stringname2) - 1;// ignore the colnames line
  ...
  cout << endl;
  printf("Start loading summary stat 2: \n");
  t1 = clock();
  Read_summarystat(stringname2, SA21, SA22, rsname2, betah2, s22, PV2, chr2, bp2, Ns2);
  
  cout << "Finish loading summary stat 2 in " << (clock() - t1)*1.0 / CLOCKS_PER_SEC << " sec." << endl;
  //----------------------------------------------------------------------------------------------//
  //mathcing panel SNP with summary stat and correct direction of minor allele
  CharacterVector rs_tmp = intersect(rsname1, rsname2);
  CharacterVector rs_inter = intersect(rs_tmp, rsname3);
