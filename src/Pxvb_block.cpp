...
List PXvbfunM2_block(arma::field<mat> F4Rblock, arma::umat block_inf, uword nblocks,
  ...
  for(int iter = 2; iter <= maxIter; iter ++){
    ...
    for(int nb2 = 0; nb2 < (int)(nblocks); nb2 = nb2 + 1){
      v2A = 1. / (1. / (F4sG2(nb2, 0))+1. / sgal2);
      F4v2A(nb2, 0) = v2A;
      int pa_block = NB[nb2];
      for (int k = 0; k < (int)pa_block; k = k + 1){
        vec tmp3;
        double RinSmuAkk;
        tmp3 = F4RinsGmuA(nb2, 0) - F4Rins2(nb2, 0).col(k)*F4muA(nb2, 0)[k];
        RinSmuAkk = F4RinsGmuA(nb2, 0)[k] - F4Rblock(nb2, 0)(k, k)*F4muA(nb2, 0)[k] / F4se2(nb2, 0)[k];
        F4muA(nb2, 0)[k] = (F4GinvsG2(nb2, 0)[k] - beta0*(1. / F4se2(nb2, 0)[k])*F4RinsGmu(nb2, 0)[k] - 1. / F4se2(nb2, 0)[k]*RinSmuAkk)*v2A[k];
        F4RinsGmuA(nb2, 0) = tmp3 + F4Rins2(nb2, 0).col(k)*F4muA(nb2, 0)[k];
      }
